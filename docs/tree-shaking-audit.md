...
1. **`core/form-reset.ts` — intentional scaffolding, kept (not deleted).** `export function onFormReset` has zero importers and is absent from the core barrel/FESM, so it never ships and is a non-issue for tree-shaking. Initially flagged "safe to delete," and that was probably right. **Correction (pass 6):** the justification for keeping it — that `calendar.ts:1151` references it as planned *"Phase 3 form-reset integration"* — is **false**. `calendar.ts` contains **zero** references to `onFormReset` at any line; it hand-rolls the same `control.events` / `FormResetEvent` subscription itself (`calendar.ts:816`). The helper has had zero importers across six audit passes, and its existence has since produced a misattribution cascade: **seven e2e specs and three docs** credited picker reset behaviour to it, including two permanently-skipped tests that *blamed* it. Reset actually works via `writeValue(null)`. Either export it and migrate `calendar.ts:816-830` onto it, or delete it — but the "deliberate groundwork" rationale should not be repeated.
