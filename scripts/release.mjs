...
//   3. Local pre-flight: lint, build:lib, verify:mcp-index, build:mcp, test:ci,
//      pack:check, verify:package.
...
// ─── 3. local pre-flight ──────────────────────────────────────────────────
step('Local pre-flight: lint, build:lib, verify:mcp-index, build:mcp, test:ci, pack:check, verify:package');
shInherit('npm run lint');
shInherit('npm run build:lib');
// The MCP index is generated by build:lib into dist/ and never committed, so it
// cannot be stale — but the guidance layer it carries is hand-authored. This
// validates coverage, cross-reference links, and every demo snippet's selectors
// and bindings against the extracted API. A wrong index is worse than no index.
shInherit('npm run verify:mcp-index');
// Assembles dist/ngx-tw-mcp from the index build:lib just produced. Proves the
// second publishable package builds *before* the tag is pushed — the release
// workflow publishes it, and a tag is far more awkward to retract than a
// failed local check.
shInherit('npm run build:mcp');
